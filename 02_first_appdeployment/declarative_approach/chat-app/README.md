...
We’ll use a custom **chat-app**  to demonstrate the true GitOps workflow.
...
> Follow this guide to set up ArgoCD: [ArgoCD Setup & Installation](../../../01_installation_setup/README.md)  
...
In below directory you can see the related manifest files:
  ...
  ```bash
  cd argocd/declarative_approach/chat-app
  ```
...
metadata:
  name: chat-app          # Name of this ArgoCD application
  namespace: argocd                # Must be created in the 'argocd' namespace
spec:
  project: default                 # ArgoCD Project (logical grouping of apps)
  source:
    repoURL: https://github.com/<your-username>/argocd.git   # Git repo containing manifests
    targetRevision: main           # Git branch or tag (e.g., main, dev, release-1.0)
    path: declarative_approach/chat-app  # Path inside repo where manifests live
  destination:
...
* You should see `chat-app`.
* Status should be **Synced** and **Healthy**.

  ![chat-app](../output_images/image-1.png)

  ![chat-app-outside](../output_images/image-3.png)

  ![chat-app-browser](../output_images/image-2.png)
...
* `chat-app` exposing port **8082**.

  ![chat-app-pod-svc](../output_images/image-4.png)
...
```bash
kubectl port-forward svc/frontend-service 8082:80 --address=0.0.0.0 &
```
...
You should see your **chat-app application UI**.

  ![online-shop-ui](../output_images/image-2.png)
...
1. Edit `backend-deployment.yml` → for example, increase replicas in the deployment manifest under `declarative_approach/online_shop`.
2. Commit & push:
