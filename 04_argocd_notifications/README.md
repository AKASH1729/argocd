...
We’ll use our **`root-app`** Application and annotate it for notifications.
...
* root App Status:
    ...
    ![root-app](output_images/image-3.png)

* root App Detail:

    ![root-app-detail](output_images/image-3.png)

* Forward the `service`(chat-app-service) of `root-app2` and access it in browser:

    ```bash
    kubectl port-forward svc/chat-app-service -n default 3000:80 --address=0.0.0.0 &
    ```
...
1. **Deploy chai-app** (with that annotations).
    ...
    * You will recieve success email on deployment success:
      ...
      ![app-healthy](output_images/image-4.png)

      ![deploy-email](output_images/image-5.png)
...
2. **Introduce a failure**:
   ...
   * Edit the Git manifest for chai-app (e.g., wrong image tag), like below (the `v1` tag does not exist):
      ...
      ![image-tag](output_images/image-5.png)
...
3. **Observe Email**:
   ...
   * Recipient (e.g., `receiver@example.com`) should receive an email with subject:
     ...
     ```
     [ArgoCD] root-app2 health is Degraded
     ```

    * Email body contains details about the failure.

      ![email](output_images/image-5.png)
